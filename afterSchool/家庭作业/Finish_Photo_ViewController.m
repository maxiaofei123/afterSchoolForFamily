...
@interface Finish_Photo_ViewController ()<UITableViewDelegate,UITableViewDataSource>
{
...
@implementation Finish_Photo_ViewController
...
-(void)homeWoorkRequest//我的作业详情请求
{
    ...
    [manager GET:[NSString stringWithFormat:@"http://114.215.125.31/api/v1/home_works?student_id=%@&work_paper_id=%@",[[NSUserDefaults standardUserDefaults]objectForKey:@"user_id"],self.photoWorkId ]parameters:nil success:^(AFHTTPRequestOperation *operation, id responseObject){
...
//        NSLog(@"workPaperRequst =%@",workPaperDic);
        ...
        NSArray * array = [NSArray arrayWithObject:index2];
        [_finishTableView reloadRowsAtIndexPaths:array withRowAnimation:UITableViewRowAnimationAutomatic];
    }failure:^(AFHTTPRequestOperation *operation, NSError *error)
     {
         NSLog(@"erro =%@",error);
...
-(void)requestTeacherRemark:(NSString * )homeWorkId
{
    ...
    [manager GET:[NSString stringWithFormat:@"http://114.215.125.31/api/v1/home_works/%@/work_review",homeWorkId ]parameters:nil success:^(AFHTTPRequestOperation *operation, id responseObject){
        ...
        NSLog(@"teacher remark =%@",teacherRemarkDic);
...
//图片 。视频 。文字 ，
    ...
    //学生的留言
        NSString * str = [myHomeWorkDic objectForKey:@"description"];
         float homeWorkHeight= [publicRequest lableSizeWidth:Main_Screen_Width-80 content:[myHomeWorkDic objectForKey:@"description"]];
        NSLog(@"学生留言 =%@",str);
        if (str.length>0) {
            UILabel * myHomeworkLable = [[UILabel alloc] initWithFrame:CGRectMake(50, 40, Main_Screen_Width-80, homeWorkHeight)];
            ...
            imageHeight = 40+homeWorkHeight;
        }
       if (homeWorkMediaArr.count > 0) {
            UIScrollView * scrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0,imageHeight+8, Main_Screen_Width-20, 100)];
            scrollView.showsHorizontalScrollIndicator = NO;
//           scrollView.backgroundColor = [UIColor yellowColor];
            scrollView.contentSize = CGSizeMake(10+108 * homeWorkMediaArr.count,100);
            //        scrollView.bounces = NO;
            scrollView.delegate =self;
            scrollView.pagingEnabled = YES;
            scrollView.userInteractionEnabled = YES;
            [cell.contentView addSubview:scrollView];
            for (int i= 0; i<[homeWorkMediaArr count]; i++) {
                    ...
                    //如果有mp4则播放按钮显示
                {
                   myimageView.backgroundColor = [UIColor colorWithRed:235/255. green:235/255. blue:235/255. alpha:1.];
                    ...
                    NSURL * imageUrl = [NSURL URLWithString:[homeWorkMediaArr objectAtIndex:i]];
                    [myimageView setImageWithURL:imageUrl];
                    myimageView.tag =i;
                    UITapGestureRecognizer *pass1 = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(homeWorkTapImage:)];
                    [myimageView addGestureRecognizer:pass1];
                }
   
            }
            imageHeight = 150 + homeWorkHeight ;
        }
    
        //老师评语
        float PingYuHeight = 0;
        UIImageView * imagePingYu = [[UIImageView alloc] initWithFrame:CGRectMake(10, imageHeight, 30, 30)];
        imagePingYu.image = [UIImage imageNamed:[NSString stringWithFormat:@"%@",@"laoshiPingyu.png"]];
        [cell.contentView addSubview:imagePingYu];
        
        UILabel * lable2 = [[UILabel alloc] initWithFrame:CGRectMake(50, imageHeight,Main_Screen_Width-80, 30)];
        lable2.text = @"老师评语";
        [cell.contentView addSubview:lable2];

        if(![teacherRemarkDic isKindOfClass:[NSNull class]])
        {
            PingYuHeight  = [publicRequest lableSizeWidth:Main_Screen_Width-80 content:[teacherRemarkDic objectForKey:@"remark"]] ;
            if ([[teacherRemarkDic objectForKey:@"rate"] isKindOfClass:[NSNull class]]) {
                UIImageView * image = [[UIImageView alloc]initWithFrame:CGRectMake(Main_Screen_Width-110, imageHeight, 70,28 ) ];
                image.image = [UIImage imageNamed:@"weiYue.png"];
                [cell.contentView addSubview:image];
            }else{
                UIButton * rateBUtton = [UIButton buttonWithType:UIButtonTypeRoundedRect];
                rateBUtton.frame = CGRectMake(Main_Screen_Width-110, imageHeight, 70,28 );
           ...
           if ([[teacherRemarkDic objectForKey:@"remark"] isKindOfClass:[NSNull class]]) {
                ...
                if (markGood.length>0) {
                    ...
                    UILabel * PingYuLable = [[UILabel alloc] initWithFrame:CGRectMake(50, 30+imageHeight, Main_Screen_Width-40-50,markGoodHeight)];
                ...
                if (markBad.length >0) {
                    ...
                    UILabel * badLable = [[UILabel alloc] initWithFrame:CGRectMake(50, 30+imageHeight+markGoodHeight , Main_Screen_Width-40-50,markBadHeight)];
     ...
     //老师语音留言
        UIImageView * imageYuYin = [[UIImageView alloc] initWithFrame:CGRectMake(10, imageHeight + PingYuHeight +35, 30, 30)];
        imageYuYin.image = [UIImage imageNamed:[NSString stringWithFormat:@"%@",@"yuyinLiuyan.png"]];
        [cell.contentView addSubview:imageYuYin];
        
        UILabel * lable3 = [[UILabel alloc] initWithFrame:CGRectMake(50, imageHeight + PingYuHeight +35 ,Main_Screen_Width-40 -50, 30)];
        lable3.text = @"老师语音留言";
        [cell.contentView addSubview:lable3];
        
        if (teachermarkMp3Str.length > 0) {
            UIView * View = [[UIView alloc] initWithFrame:CGRectMake(10,imageHeight + PingYuHeight +35+40, Main_Screen_Width-40, 100)];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.section ==1) {
        ...
        if (teachermarkMp3Str.length >0) {
         ...
         return 120+ homeWorkHeight + PingYuHeight+remarkHeight+mediaheight;
    }
    float titleLableSizeHeight= [publicRequest lableSizeWidth:Main_Screen_Width-40 content:[workPaperDic objectForKey:@"title"]] ;
    float contentLableSizeHeight= [publicRequest lableSizeWidth:Main_Screen_Width-40 content:[workPaperDic objectForKey:@"description"]] ;
    int  imageheight = 0 ;
